...
main(int argc, char *argv[]) {
	...
	if (activate_lease6(pool, &iaaddr, &ds, 1) != ISC_R_SUCCESS) {
	...
	for (i=10; i<100; i+=10) {
		if (activate_lease6(pool, &iaaddr, &ds, i) != ISC_R_SUCCESS) {
			printf("ERROR: activate_lease6() %s:%d\n", MDL);
			return 1;
		}
		if (iaaddr_dereference(&iaaddr, MDL) != ISC_R_SUCCESS) {
	...
	if (activate_lease6(pool, &iaaddr, &ds, 42) != ISC_R_SUCCESS) {
		printf("ERROR: activate_lease6() %s:%d\n", MDL);
		return 1;
	}
	if (iaaddr_dereference(&iaaddr, MDL) != ISC_R_SUCCESS) {
	...
	if (activate_lease6(pool, &iaaddr, &ds, 11) != ISC_R_SUCCESS) {
		printf("ERROR: activate_lease6() %s:%d\n", MDL);
		return 1;
	}
	if (iaaddr_dereference(&iaaddr, MDL) != ISC_R_SUCCESS) {
	...
	if (activate_lease6(pool, &iaaddr, &ds, 11) != ISC_R_NORESOURCES) {
		printf("ERROR: activate_lease6() %s:%d\n", MDL);
		return 1;
	}
	if (ipv6_pool_dereference(&pool, MDL) != ISC_R_SUCCESS) {
