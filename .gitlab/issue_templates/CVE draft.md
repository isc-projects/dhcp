...
Internal notes, like this one, will give directions for using this template.  Delete these before publication.

(Internal) (Hidden Draft)
The version in the KB is the authoritative Advisory, this is only a DRAFT text. To create a new advisory text article in the KB, first create a new blank article with a non-descript subject in the Security Advisories section.  Second, mark the article as hidden and move it to the top of the list so that it can be found easily.  Third, after verifying that the article is hidden, modify the article "slug" below the title to match the CVE identifier (you should not do the redirect if prompted).  Fourth, copy and paste the entire template contents (that you're reading right now) from the template article into the new article.  Fifth, remove this note from the new article.
...
**Acknowledgments:** ISC would like to thank Phineas Q. Researcher for discovering and reporting this issue.
